# efect_proxy.py
# EFECT Proxy + ChatGPT-like Web UI (single-file) — FIXED (no JS template literals inside Python strings)
...
EXE_URL = os.getenv("EXE_URL", "").strip()
...
def _is_valid_token(token: str) -> bool:
    ...
    if token in TOKENS_ALLOWLIST:
        return True
    store = _token_store()
    tok = store.get("tokens", {}).get(token)
    if not tok:
        return False
    # allow future revocation if you want
    if isinstance(tok, dict) and tok.get("revoked") is True:
        return False
    return True
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    body{
      margin:0;
      background:radial-gradient(1200px 600px at 50% 20%, rgba(99,255,115,.10), transparent 60%), var(--bg);
      color:var(--text); font-family:var(--sans); height:100vh; overflow:hidden;
    }
    .app{display:flex;height:100vh}
    .sidebar{
      width:320px;
      background:linear-gradient(180deg, rgba(15,16,16,.95), rgba(10,10,10,.95));
      border-right:1px solid var(--border);
      padding:16px; display:flex; flex-direction:column; gap:12px;
    }
    .brand{
      padding:14px; border:1px solid #1d1d1d; border-radius:16px;
      background:rgba(0,0,0,.35); box-shadow:var(--shadow);
    }
    .brand h1{margin:0;font-size:22px;letter-spacing:.08em;color:var(--green);text-shadow:0 0 18px rgba(99,255,115,.22)}
    ...
    .dot{width:10px;height:10px;border-radius:999px;background:var(--green);box-shadow:0 0 14px rgba(99,255,115,.65)}
    .title{font-weight:650;letter-spacing:.04em}
    .right{margin-left:auto;display:flex;gap:10px;align-items:center}
    .link{color:var(--green);text-decoration:none;font-size:13px}
    ...
    .pill{font-size:11px;padding:3px 8px;border:1px solid #2a2a2a;border-radius:999px;background:rgba(255,255,255,.02)}
    pre{
      background:rgba(0,0,0,.45); border:1px solid #2a2a2a; padding:10px; border-radius:12px; overflow:auto;
      font-family:var(--mono); font-size:12.5px;
    }
    code{font-family:var(--mono)}
    .composer{border-top:1px solid var(--border); padding:14px; background:rgba(0,0,0,.25)}
    .composer .wrap{max-width:1100px;margin:0 auto;display:flex;gap:10px;align-items:flex-end}
    textarea{
      flex:1; min-height:48px; max-height:180px; resize:none;
      background:rgba(0,0,0,.35); border:1px solid #2a2a2a; color:var(--text);
      padding:12px; border-radius:14px; outline:none; font-family:var(--sans);
    }
    ...
    .notice{max-width:1100px;margin:10px auto 0;color:var(--muted);font-size:12px;display:flex;gap:10px;flex-wrap:wrap}
  </style>
</head>
<body>
  <div class="app">
    <div class="sidebar">
      <div class="brand">
        <h1>EFECT AI</h1>
        <div class="sub">Browser chat UI (streaming).<br/>Token-gated proxy + uploads.</div>
      </div>
      ...
      <div>
        ...
        <input class="input" id="model" placeholder="gpt-4.1-mini" value="gpt-4.1-mini"/>
      </div>
...
function renderMarkdownish(text){
  ...
  let out = esc.replace(/```([\\s\\S]*?)```/g, function(m, p1){
...
function newChat(){
  history = [];
  chat.innerHTML = "";
  addMsg("assistant", "EFECT AI online. Drop your question.");
}
...
async function streamChat(userText){
  ...
  if (!token){
    addMsg("assistant", "Missing token. Put your token in the left sidebar.");
    return;
  }
  ...
  const payload = {
    model: model,
    input: history.concat([{role:"user", content:userText}]).map(function(m){
...
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
  <title>EFECT AI — Token Generator</title>
  <style>
    body {
...
<body>
  <div class="box">
    ...
    <p>
      ...
      Protected by an invite code so random people can’t mint unlimited tokens.
    </p>
...
def _html_download_page() -> str:
    if EXE_URL:
        btn = (
            '<p><a href="' + EXE_URL +
            '" style="color:#0b0b0b;background:#63ff73;padding:10px 14px;'
            'border-radius:12px;display:inline-block;text-decoration:none;font-weight:700;">'
            'Download EFECT AI Desktop App</a></p>'
        )
    else:
        btn = "<p><b>EXE_URL is not set</b>. Add EXE_URL in Render env vars to your GitHub Release asset URL.</p>"

    return (
        "<!doctype html><html><head><meta charset='utf-8'/>"
        "<meta name='viewport' content='width=device-width,initial-scale=1'/>"
        "<title>EFECT AI — Download</title>"
        "<style>"
        ...
        "a{color:#63ff73}"
        "</style></head><body>"
        "<div class='box'><h1>EFECT AI DESKTOP</h1>"
        "<p>Download the EFECT AI desktop app.</p>" + btn +
            ...
            with requests.post(
                ...
                for raw in r.iter_lines(decode_unicode=True):
                    ...
                    if raw.startswith("data:"):
                        ...
                        # Fallback: ignore noisy non-data lines
                        continue
        except requests.RequestException as e:
...
async def upload(req: Request, file: UploadFile = File(...)) -> JSONResponse:
    ...
    if ext in (".png", ".jpg", ".jpeg", ".webp", ".bmp"):
        ...
        return JSONResponse({"kind": "image", "name": filename, "data_url": "data:" + mime + ";base64," + b64})
